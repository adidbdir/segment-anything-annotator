"""Persistent subprocess client for the isolated MatSAM worker."""
...
MATSAM_ENV = REPO_ROOT / "envs" / "matsam"
MATSAM_PYTHON = MATSAM_ENV / "bin" / "python"
WORKER_PATH = REPO_ROOT / "auto_annotation" / "matsam_worker.py"
...
class MatSamClientError(RuntimeError):
    ...
    def __init__(
        ...
        *,
        ...
        message = f"MatSAM {command} failed ({category}): {detail}"
        if stderr_tail:
            message = f"{message}\nworker stderr tail:\n{stderr_tail}"
...
class MatSamProtocolError(MatSamClientError):
    """Raised when the JSONL stream is malformed or desynchronized."""


class MatSamTimeoutError(MatSamClientError):
    """Raised when a worker command exceeds its deadline."""


class MatSamRemoteError(MatSamClientError):
...
def matsam_interpreter_exists() -> bool:
    """Return whether the isolated MatSAM interpreter exists on disk."""
    return MATSAM_PYTHON.is_file()


def _encode_image(image_np: np.ndarray) -> JsonObject:
    image = np.asarray(image_np)
    if image.dtype != np.uint8:
        raise ValueError("MatSAM images must have dtype uint8")
    if image.ndim != 3 or image.shape[2] != 3 or min(image.shape[:2]) <= 0:
        raise ValueError("MatSAM images must have shape [H, W, 3]")
...
def _encode_image_key(image_key: Any) -> JsonObject:
    if image_key.tile_settings is not None:
        raise ValueError("MatSAM Phase 4 does not support tile settings")
    return {
...
class MatSamWorkerClient:
    """Serialize RPCs to one persistent isolated MatSAM process."""

    def __init__(self) -> None:
        if not matsam_interpreter_exists():
            raise FileNotFoundError(f"MatSAM interpreter is missing: {MATSAM_PYTHON}")
        ...
        if (
            ...
            self._process.kill()
            raise RuntimeError("failed to create MatSAM worker pipes")
    ...
    def __del__(self) -> None:
        ...
        try:
            if getattr(self, "_process", None) is None or not hasattr(self, "_closed"):
                return
            self.terminate()
        except Exception:
            pass
    ...
    def probe(self, sam_variant: str, checkpoint_path: str) -> JsonObject:
        """Return whether the requested checkpoint exists locally."""
    ...
    def init(
        ...
        """Load a MatSAM SAM-v1 model and return worker metadata."""
        return self._request(
            "init",
            {
                "sam_variant": sam_variant,
    ...
    def set_image(self, image_np: np.ndarray, image_key: Any) -> JsonObject:
        """Send the defensive set_image command, which MatSAM rejects."""
    ...
    def predict(
        ...
        *,
        ...
        """Send the defensive predict command, which MatSAM rejects."""
        return self._request(
            "predict",
            {
                "point_coords": (
    ...
    def generate_auto(
        ...
        """Run MatSAM automatic segmentation and return polygon records."""
        started_at = time.monotonic()
        return self._request(
            "generate_auto",
            {
    ...
    def _parse_response(
        ...
        if response.get("protocol") != PROTOCOL_VERSION:
            self._raise_fatal(
                command, "protocol_mismatch", "unexpected protocol version"
            )
        if response.get("id") != request_id:
            self._raise_fatal(command, "id_mismatch", "response ID did not match")
        if response.get("cmd") != command:
            self._raise_fatal(
                command, "command_mismatch", "response command did not match"
            )
        if response.get("ok") is True:
            ...
            if not isinstance(result, dict):
                self._raise_fatal(
                    command, "invalid_response", "result must be an object"
                )
            return result
        ...
        remote_error = MatSamRemoteError(
            command,
            error["code"],
            error["type"],
            error["message"],
